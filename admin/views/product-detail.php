...
<div class="product-details">
		...
		<tr>
			...
			<td>
				<select name="category" id="select-category">
					...
					<?php $categories = fetch_functions\get_rows('categories');
	...
	<form action="../../functions/process-forms.php" method="post">
		...
		<button class="pull-right custom-btn" type="submit" name="add-category">Insert</button>
	</form>
	<br><br>


	<form action="../../functions/process-forms.php" method="post" id="approve-req">
		<input type="hidden" name="product-code" value="<?php echo $product_code;?>">
		<input type="hidden" name="set-category" value="">
		<button type="submit" name = "reject" class="pull-right custom-btn">Reject</button>
		<button type="submit" name = "approve" class="pull-right custom-btn" style="margin-right: 5px;">Approve</button>
	</form>
...
<script>
	...
	$('#select-category').on('change',function(){
		var val = $(this).val();
		$('#approve-req input').last().val(val);
	})
