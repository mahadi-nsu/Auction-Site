...
$category="Choose Category"; $title=""; $seller="";
if(isset($_POST['search-in-auctions'])){
	...
	$rows = fetch_functions\get_row('sell_posts','approval',1);
}
$count = sizeof($rows);
...
<div class="auctions-top">
	...
	<form action="" method="POST" id="search-auctions">
		<select name = "category">
			...
			<?php 
			$categories = fetch_functions\get_rows('categories');
...
<section class="products-wrapper">
	...
	for($i = $count-1; $i>=0; $i--) { 
		...
		$code = fetch_functions\fix_product_code($rows[$i]->product_code);
